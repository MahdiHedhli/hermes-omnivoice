...
The `current` command now also rejects symlinked selection files before reading
local voice state.
The voice helper `list` command now rejects symlinked voice registry roots
before enumerating local profile names.
...
file.
The voice helper preview command now preserves the final output path when it
launches the wrapper, so preview output symlinks are replaced instead of
followed.
...
Docker can run.
The Python adapter now rejects invalid speed and sample-rate bounds before
...
loading the OmniVoice backend.
The Python environment setup helper now rejects empty model and package values
before export or pip command planning, and auto-CLI mode rejects empty models
before backend command construction.
...
The bridge has now been installed on the homelab Hermes host outside the
Hermes source checkout at `/opt/hermes-local-tts/omnivoice-bridge`, with an
isolated OmniVoice Python runtime under the `claude` account. Live Hermes
configuration includes an available `omnivoice` command provider using the
`homelab_narrator` designed profile, while the active production provider
remains `xtts-v2`.

## Latest heartbeat

- Time: 2026-06-01 16:30 America/New_York
- Completed:
  - Confirmed the branch was clean at heartbeat start.
  - Used the homelab Hermes path instead of Lume: Mac Studio SSH hop to
    `hermes-01` at `100.114.4.26`.
  - Installed the bridge runtime to
    `/opt/hermes-local-tts/omnivoice-bridge` as `claude`, outside the dirty
    Hermes source checkout.
  - Created a consent-confirmed designed profile `homelab_narrator` under
    `/home/claude/.hermes/voices/omnivoice`.
  - Installed the isolated OmniVoice Python runtime at
    `/home/claude/.cache/hermes/omnivoice-python`; no Studio service was
    exposed.
  - Added `tts.providers.omnivoice` to live Hermes config with the real Python
    adapter command and a 600 second timeout, while preserving active
    `tts.provider: xtts-v2`.
  - Proved Hermes' own `tools.tts_tool.text_to_speech_tool` can synthesize via
    the `omnivoice` command provider and return a valid Opus file through the
    existing voice-compatible conversion path.
  - Confirmed remote Hermes source git status still only shows pre-existing
    homelab changes; the bridge install did not modify the source checkout.
- Commands:
  - `git status --short --branch`
  - `python3 scripts/install-hermes-omnivoice-bridge.py --target-root /tmp/hermes-omnivoice-bridge-package --with-examples --update-gitignore --force`
  - `scripts/create-omnivoice-voice.py design homelab_narrator ... --confirm-consent --force`
  - `scripts/setup-omnivoice-python-env.py --json`
  - `scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `scripts/test-omnivoice-tts.sh`
  - `tools.tts_tool.text_to_speech_tool(...)` with in-memory provider override
    to `omnivoice`
  - `git status --short --branch` in `/opt/hermes-agent/source` as `claude`
- Tests:
  - Homelab installed acceptance with `--require-real-backend`: PASS for
    static MVP readiness, real backend readiness, and Hermes source readiness.
  - Homelab installed `scripts/test-omnivoice-tts.sh`: PASS, generated a valid
    temporary WAV from the required smoke text using the real Python backend.
  - Live Hermes command-provider smoke: PASS, returned
    `/tmp/.../hermes-omnivoice-real-smoke.ogg` identified as Opus audio at
    24000 Hz.
  - Manual real wrapper synthesis with 600 second timeout: PASS in about 52
    seconds, generated 16-bit mono WAV at 24000 Hz.
- Blockers:
  - Live OmniVoice-Studio API is not running on `hermes-01`; the proven route is
    the local Python adapter using voices created/imported into the Hermes
    registry.
  - Native provider work remains deferred because the live Hermes source tree
    is already dirty with unrelated homelab changes and the command-provider
    MVP is working.
- Assumptions:
  - Keeping `xtts-v2` as the active provider is the safer rollout posture; an
    operator can switch to `omnivoice` after reviewing latency and disk impact.
  - The designed `homelab_narrator` profile is acceptable for validation
    because it uses no cloned reference sample and has explicit consent
    metadata.
- Next action:
  - Decide whether to switch live Hermes `tts.provider` to `omnivoice` for an
    operator trial, or keep it as an available provider and add a small
    documented voice-selection workflow.
...
- Completed:
  ...
  - Hardened `scripts/hermes-omnivoice-voices.py current` so symlinked
    selection files are rejected before JSON is read or profile state is
    resolved.
  - Added regression coverage proving a valid selection symlink fails closed
    instead of being followed.
  - Updated custom voice, README, MVP handoff, weekend-summary, and heartbeat
    docs for the selection-file symlink read guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -12`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests.test_current_command_rejects_selection_file_symlink tests.test_omnivoice_tts.VoiceCliTests.test_set_command_replaces_selection_symlink_without_touching_target tests.test_omnivoice_tts.VoiceCliTests.test_set_and_current_commands_manage_selection_file tests.test_omnivoice_tts.VoiceCliTests.test_current_command_refuses_malformed_selection_shape -v`
...
- Tests:
  - Focused selection-file symlink read guard suite: PASS, 4 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 206 tests with 1 skipped
...
- Assumptions:
  - Local selection metadata is non-authoritative pointer state; symlinked
    selection files should be treated as local file aliases and refused before
    read-side current-state resolution.
- Next action:
...
- Time: 2026-06-01 09:00 America/New_York
- Completed:
  - Confirmed the branch was clean at heartbeat start.
  - Rechecked remaining blockers; actual Hermes Agent source is still absent,
    so native-provider work remains deferred.
  - Hardened `scripts/hermes-omnivoice-voices.py list` so symlinked
    `--voices-dir` roots are rejected before profile names are enumerated.
  - Added regression coverage proving the list command fails closed for a
    symlinked registry root.
  - Updated custom voice, README, MVP handoff, weekend-summary, and heartbeat
    docs for the helper list registry-root guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -10`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests.test_list_command_rejects_voices_root_symlink tests.test_omnivoice_tts.VoiceCliTests.test_list_profiles_marks_ready_and_invalid_entries tests.test_omnivoice_tts.RuntimeCheckTests.test_runtime_check_rejects_symlink_voices_root tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_symlink_voices_root_fails -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused helper list registry-root guard suite: PASS, 4 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 205 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Voice registry root symlinks are a trust-boundary violation even for read
    oriented helper commands because listing can disclose profile names from an
    aliased registry.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Rechecked remaining blockers; actual Hermes Agent source is still absent,
    so native-provider work remains deferred.
  - Hardened `scripts/hermes-omnivoice-voices.py preview` so it preserves the
    final output path when invoking the wrapper instead of resolving through an
    existing output symlink.
  - Added regression coverage proving preview replaces the output symlink and
    leaves the symlink target untouched while still producing a valid WAV.
  - Updated setup, custom voice, README, MVP handoff, weekend-summary, and
    heartbeat docs for the preview output symlink guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests.test_preview_replaces_output_symlink_without_touching_target tests.test_omnivoice_tts.VoiceCliTests.test_preview_generates_audio_with_fake_backend tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_command_backend_replaces_output_symlink_without_touching_target tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_command_success_writes_valid_wav -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused preview output symlink guard suite: PASS, 4 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 203 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Preview output should use the same final-path symlink replacement contract
    as direct wrapper synthesis.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Hardened runtime diagnostics so local voice readiness reuses the wrapper
    voice-profile validator instead of counting metadata files alone.
  - Added regression coverage proving unsafe registry aliases and cloned
    `ref_audio` symlinks are ignored and acceptance does not report
    `voices_ready` from them.
  - Updated setup, acceptance, README, MVP handoff, weekend-summary, and
    heartbeat docs for the runtime voice-readiness guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `python3 -m unittest tests.test_omnivoice_tts.RuntimeCheckTests.test_runtime_check_rejects_symlink_voices_root tests.test_omnivoice_tts.RuntimeCheckTests.test_runtime_check_ignores_symlink_voice_profiles_for_readiness tests.test_omnivoice_tts.RuntimeCheckTests.test_runtime_check_ignores_clone_ref_audio_symlink_for_readiness tests.test_omnivoice_tts.RuntimeCheckTests.test_runtime_check_reports_missing_backend_without_failing tests.test_omnivoice_tts.AcceptanceTests.test_acceptance_can_require_real_backend -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused runtime voice-readiness guard suite: PASS, 5 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 198 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Runtime readiness should not be looser than synthesis-time registry
    validation; unsafe local aliases are diagnostic evidence, not usable voice
    profiles.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Time: 2026-06-01 06:00 America/New_York
- Completed:
  - Confirmed the branch was clean at heartbeat start.
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened runtime profile loading so final voice-directory symlinks are
    rejected before resolution, including aliases that point back inside the
    registry root.
  - Added regression coverage proving in-root voice-directory aliases fail
    closed while existing escape, metadata, reference-audio, and valid clone
    profile tests still pass.
  - Updated setup, custom voice, README, MVP handoff, weekend-summary, and
    heartbeat docs for the expanded runtime registry symlink guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `python3 -m unittest tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_symlink_voice_dir_alias_fails tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_symlink_voice_dir_escape_fails tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_voice_yaml_symlink_fails tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_ref_audio_symlink_fails tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_loads_valid_clone_voice -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused runtime registry symlink guard suite: PASS, 5 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 195 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Local registry material is a trust boundary; helper-created profile
    directories and files are private regular filesystem objects, so runtime
    symlink rejection is compatible with the documented MVP flow.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Time: 2026-06-01 05:30 America/New_York
- Completed:
  - Confirmed the branch was clean at heartbeat start.
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened runtime profile loading so symlinked `voice.yaml` files and cloned
    `ref_audio` files are rejected before synthesis uses local registry
    material.
  - Added regression coverage proving symlinked registry metadata and reference
    audio fail closed while valid clone profiles still load and synthesize.
  - Updated setup, custom voice, README, MVP handoff, weekend-summary, and
    heartbeat docs for the runtime registry symlink guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused runtime registry symlink guard suite: PASS, 4 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 194 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Local registry material is a trust boundary; helper-created files are
    private regular files, so runtime symlink rejection is compatible with the
    documented MVP flow.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened runtime Studio readiness checks so malformed `/profiles` JSON is
    reported as `invalid`, not `reachable`.
  - Added regression coverage proving malformed Studio profile-list payloads do
    not count as Studio-ready or backend-ready in acceptance evaluation.
  - Updated setup, Studio bridge, README, MVP handoff, weekend-summary, and
    heartbeat docs for the runtime payload-shape guard.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Focused runtime Studio payload-shape suite: PASS, 3 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 189 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Assumptions:
  - A reachable Studio endpoint is not sufficient evidence of backend readiness
    unless the profile-list payload shape is usable.
- Next action:
  - Continue bounded MVP hardening or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened `scripts/setup-omnivoice-python-env.py` so empty `--model` and
    `--package` values fail before export or pip command planning.
  - Hardened wrapper auto-CLI mode so an empty model fails before backend
    command construction.
  - Updated custom voice, handoff, and weekend-summary docs for the setup and
    auto-CLI model validation and 174-test snapshot.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -10`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `python3 -m unittest tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_auto_cli_builds_official_design_command tests.test_omnivoice_tts.OmniVoiceRegistryTests.test_auto_cli_rejects_empty_model_before_command tests.test_omnivoice_tts.PythonEnvSetupTests.test_setup_env_shell_prints_exportable_environment tests.test_omnivoice_tts.PythonEnvSetupTests.test_setup_env_rejects_empty_model_before_plan tests.test_omnivoice_tts.PythonEnvSetupTests.test_setup_env_rejects_empty_package_before_plan -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  - `git diff --check`
- Tests:
  - Focused setup/auto-CLI validation suite: PASS, 5 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 174 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Setup-generated exports and auto-CLI fallback commands should reject empty
    model/package configuration before any backend or installer command runs.
- Next action:
  - Continue bounded MVP hardening, or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened `scripts/hermes-omnivoice-python-adapter.py` so invalid
    backend-reported sample rates fail through concise adapter errors instead
    of raw conversion exceptions.
  - Updated custom voice, handoff, and weekend-summary docs for the backend
    sample-rate validation and 168-test snapshot.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -10`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `git diff --check`
- Tests:
  - Focused Python adapter suite: PASS, 8 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 168 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - Backend-reported sample rates should be validated before writing audio so
    adapter failures stay diagnosable and do not leak raw conversion errors.
- Next action:
  - Continue bounded MVP hardening, or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Time: 2026-05-31 23:00 America/New_York
- Completed:
  - Confirmed the branch was clean at heartbeat start.
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened `scripts/hermes-omnivoice-python-adapter.py` so invalid
    `--speed` and `--sample-rate` values fail before OmniVoice backend loading.
  - Updated custom voice, handoff, and weekend-summary docs for the new adapter
    bounds validation and 166-test snapshot.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  ...
  - `git diff --check`
- Tests:
  - Focused Python adapter bounds suite: PASS, 6 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 166 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - The Python adapter may be invoked directly through command-provider config,
    so it should enforce basic synthesis bounds before loading model code.
- Next action:
  - Continue bounded MVP hardening, or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Completed:
  ...
  - Rechecked default OmniVoice runtime diagnostics and Hermes source discovery;
    default runtime remains intentionally unconfigured and no actual Hermes
    Agent checkout was found.
  - Hardened `scripts/omnivoice-studio-local.py` so negative
    `--command-timeout` values fail before Docker or Git commands can run,
    while `--command-timeout 0` remains the explicit unbounded manual escape
    hatch.
  - Updated Studio setup/bridge and handoff docs for the new command-timeout
    validation and 159-test snapshot.
- Commands:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `python3 -m unittest tests.test_omnivoice_tts.StudioLocalTests.test_command_timeout_must_not_be_negative tests.test_omnivoice_tts.StudioLocalTests.test_cli_rejects_negative_command_timeout_before_subcommand tests.test_omnivoice_tts.StudioLocalTests.test_command_timeout_zero_remains_unbounded tests.test_omnivoice_tts.StudioLocalTests.test_run_command_timeout_fails_cleanly -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  - `git diff --check`
- Tests:
  - Focused Studio command-timeout suite: PASS, 4 tests.
  - `scripts/validate-omnivoice-bridge.sh`: PASS, 159 tests with 1 skipped
    opt-in real-backend unittest; fake smoke PASS; unconfigured default smoke
    SKIP.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - `scripts/test-omnivoice-tts.sh` with prepared Python adapter exports: PASS,
...
- Blockers:
  ...
  - Default shell runtime is still unconfigured until the prepared Python
    adapter exports are evaluated.
- Assumptions:
  - `--command-timeout 0` is the only intended unbounded Studio helper mode;
    negative values should not silently disable subprocess deadlines.
- Next action:
  - Continue bounded MVP hardening, or switch to native-provider work when an
    actual Hermes Agent checkout is available.
...
- Time: 2026-05-31 06:00 America/New_York
- Completed:
  - Rechecked repo state; branch was clean at commit `c8153d2`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Reviewed current selection-state helper behavior and docs after the 05:30
    stale-selection validation checkpoint.
  - Updated `scripts/hermes-omnivoice-voices.py current` to require a JSON
    object selection payload and `provider: omnivoice` before resolving the
    selected voice.
  - Added regression coverage for non-object selection JSON and foreign
    provider values so malformed or unrelated selection files fail closed.
  - Updated custom voice docs, the packaged MVP handoff, and weekend summary to
    document the stricter selection metadata gate.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -6`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `rg -n "Latest heartbeat|Open follow-ups|Next action|current|selection|provider|omnivoice-selection|TODO|FIXME|Remaining Blockers|Next Steps" HEARTBEAT.md README.md docs scripts tests examples`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests -v`
  - `python3 -m py_compile scripts/hermes-omnivoice-voices.py tests/test_omnivoice_tts.py`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__`
  - `rg -n "05:30 America/New_York|05:00 America/New_York|04:30 America/New_York|101 tests|100 tests|99 tests|98 tests" docs/omnivoice-mvp-handoff.md docs/omnivoice-weekend-summary.md README.md docs/omnivoice-acceptance.md`
  - `git diff --check`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Targeted voice helper tests: PASS, 12 tests.
  - Python py_compile for the voice helper and tests: PASS.
  - `scripts/validate-omnivoice-bridge.sh`: PASS; includes 103 tests with 1
    expected opt-in real-backend skip, py_compile, strict package-file
    acceptance, fake-backend smoke, unconfigured smoke skip, secret-pattern
    scan, helper-backed generated-artifact scan, and `git diff --check`.
  - Strict real-backend acceptance after evaluating generated shell exports:
    PASS; `real_backend_ready: true`, `hermes_source_ready: false`,
    `package_files.required_count: 7`.
  - Stale handoff snapshot scan: PASS; no 05:30/101-test or older summary state
    remains in the current handoff docs.
  - Repo artifact scan: PASS; no generated audio, models, local voice samples,
    env files, caches, or local selection state found.
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - Local selection JSON is untrusted user-level state and should prove it
    belongs to OmniVoice before any voice readiness is reported.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
  - Commit the selection metadata provider guard and keep the branch clean for
    handoff, or install the bridge into the real Hermes Agent checkout once the
    source path is available.

## Previous heartbeat

- Time: 2026-05-31 06:30 America/New_York
- Completed:
  - Rechecked repo state; branch was clean at commit `29734f5`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Reviewed current selection-state code and tests after the 06:00 metadata
    validation checkpoint.
  - Updated `scripts/hermes-omnivoice-voices.py current` to report
    profile-derived `speed` and normalized `voices_dir` after validation,
    instead of trusting possibly stale selection-file fields.
  - Added regression coverage that a stale selection-file speed is ignored in
    favor of the revalidated voice profile speed.
  - Updated custom voice docs, the packaged MVP handoff, and weekend summary to
    document the profile-derived current-state behavior.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -6`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `sed -n '135,210p' scripts/hermes-omnivoice-voices.py`
  - `sed -n '2510,2645p' tests/test_omnivoice_tts.py`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests -v`
  - `python3 -m py_compile scripts/hermes-omnivoice-voices.py tests/test_omnivoice_tts.py`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__`
  - `rg -n "06:00 America/New_York|05:30 America/New_York|05:00 America/New_York|103 tests|101 tests|100 tests|99 tests" docs/omnivoice-mvp-handoff.md docs/omnivoice-weekend-summary.md README.md docs/omnivoice-acceptance.md`
  - `git diff --check`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Targeted voice helper tests: PASS, 13 tests.
  - Python py_compile for the voice helper and tests: PASS.
  - `scripts/validate-omnivoice-bridge.sh`: PASS; includes 104 tests with 1
    expected opt-in real-backend skip, py_compile, strict package-file
    acceptance, fake-backend smoke, unconfigured smoke skip, secret-pattern
    scan, helper-backed generated-artifact scan, and `git diff --check`.
  - Strict real-backend acceptance after evaluating generated shell exports:
    PASS; `real_backend_ready: true`, `hermes_source_ready: false`,
    `package_files.required_count: 7`.
  - Stale handoff snapshot scan: PASS; no 06:00/103-test or older summary state
    remains in the current handoff docs.
  - Repo artifact scan: PASS; no generated audio, models, local voice samples,
    env files, caches, or local selection state found.
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - User-level selection files are pointers to voice profiles; current voice
    state should be derived from the validated profile rather than cached
    selection fields.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
  - Commit the profile-derived current-state guard and keep the branch clean for
    handoff, or install the bridge into the real Hermes Agent checkout once the
    source path is available.
...
- Completed:
  - Rechecked repo state; branch was clean at commit `de2936e`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Reviewed selection-file write handling after the live current-state guard.
  - Added `write_selection_file()` so `scripts/hermes-omnivoice-voices.py set`
    writes through a same-directory temporary file, applies `0600`
    permissions, and atomically replaces the destination.
  - Stopped resolving the final selection path before write, so an existing
    destination symlink is replaced instead of followed.
  - Added regression coverage for private selection-file permissions and for
    replacing a destination symlink without modifying its target.
  - Updated custom voice docs, the packaged MVP handoff, and weekend summary to
    document private atomic selection-state writes.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -6`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `rg -n "write_text\\(|selection_path|selection-file|omnivoice-selection|chmod|permission|atomic|replace\\(" scripts tests docs README.md HEARTBEAT.md`
  - `python3 -m unittest tests.test_omnivoice_tts.VoiceCliTests -v`
  - `python3 -m py_compile scripts/hermes-omnivoice-voices.py tests/test_omnivoice_tts.py`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__`
  - `rg -n "06:30 America/New_York|06:00 America/New_York|05:30 America/New_York|104 tests|103 tests|101 tests" docs/omnivoice-mvp-handoff.md docs/omnivoice-weekend-summary.md README.md docs/omnivoice-acceptance.md`
  - `git diff --check`
  - `python3 scripts/check-omnivoice-artifacts.py --json`
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Targeted voice helper tests: PASS, 15 tests.
  - Python py_compile for the voice helper and tests: PASS.
  - `scripts/validate-omnivoice-bridge.sh`: PASS; includes 106 tests with 1
    expected opt-in real-backend skip, py_compile, strict package-file
    acceptance, fake-backend smoke, unconfigured smoke skip, secret-pattern
    scan, helper-backed generated-artifact scan, and `git diff --check`.
  - Strict real-backend acceptance after evaluating generated shell exports:
    PASS; `real_backend_ready: true`, `hermes_source_ready: false`,
    `package_files.required_count: 7`.
  - Stale handoff snapshot scan: PASS; no 06:30/104-test or older summary state
    remains in the current handoff docs.
  - Repo artifact scan: PASS; no generated audio, models, local voice samples,
    env files, caches, or local selection state found.
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - Local selection metadata is user-level state and should be written
    defensively even though it should not contain secrets.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
...
- Completed:
  - Rechecked repo state; branch was clean at commit `0287d1c`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Re-ran bounded Hermes source discovery; it still sees only this bridge repo,
    not an actual Hermes Agent checkout.
  - Audited source discovery timeout handling and found that
    `--scan-timeout 0` disabled the deadline.
  - Added source discovery validation so non-positive scan timeouts fail before
    walking the filesystem.
  - Added CLI and acceptance coverage proving invalid scan timeouts return
    concise helper-prefixed errors without tracebacks.
  - Updated acceptance, setup, MVP handoff, weekend summary, and heartbeat docs
    for the new 150-test validation snapshot.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `rg -n "TODO|FIXME|Next action|Open follow-ups|timeout|urlopen|request\\.urlopen|--timeout|command-timeout|consent|allowed_uses|allowed-use" HEARTBEAT.md README.md docs scripts tests examples`
  - `rg -n "scan-timeout|source-scan-timeout|bounded|unbounded" README.md docs HEARTBEAT.md scripts tests`
  - `python3 -m unittest tests.test_omnivoice_tts.HermesSourceFinderTests.test_discovery_refuses_non_positive_scan_timeout tests.test_omnivoice_tts.HermesSourceFinderTests.test_discovery_cli_invalid_scan_timeout_fails_without_traceback tests.test_omnivoice_tts.AcceptanceTests.test_acceptance_invalid_source_scan_timeout_fails_without_traceback tests.test_omnivoice_tts.HermesSourceFinderTests.test_scores_likely_hermes_source_with_tts_files -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Source discovery timeout focused suite: PASS, 4 tests.
  - Full bridge validator: PASS, 150 tests, 1 skipped real-backend unittest
    smoke by default.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - Real-backend smoke script with prepared Python adapter exports: PASS,
    generated a valid temporary WAV.
...
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - Source discovery should remain bounded during automation; a future
    unbounded search should be a separate operator-supervised mode if needed.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
  - Continue bounded MVP hardening or install the bridge into the real Hermes
...
- Time: 2026-05-31 18:30 America/New_York
- Completed:
  - Rechecked repo state; branch was clean at commit `fd1aa56`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Re-ran bounded Hermes source discovery; it still sees only this bridge repo,
    not an actual Hermes Agent checkout.
  - Audited local Studio helper timeout handling.
  - Added Studio health probe timeout validation so non-positive `--timeout`
    values fail before `urllib.request.urlopen`.
  - Added focused coverage proving invalid health timeouts fail closed and
    `--command-timeout 0` still maps to an intentionally unbounded Docker/Git
    subprocess timeout.
  - Updated setup, Studio bridge, MVP handoff, weekend summary, and heartbeat
    docs for the new 147-test validation snapshot.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -8`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `python3 scripts/find-hermes-source.py --json`
  - `rg -n "Latest heartbeat|Next action|Open follow-ups|--timeout|command-timeout|scan-timeout|validate_timeout|urlopen|timeout=" HEARTBEAT.md docs scripts tests README.md examples`
  - `sed -n '1,220p' scripts/omnivoice-studio-local.py`
  - `sed -n '2080,2405p' tests/test_omnivoice_tts.py`
  - `python3 -m unittest tests.test_omnivoice_tts.StudioLocalTests.test_health_timeout_must_be_positive tests.test_omnivoice_tts.StudioLocalTests.test_command_timeout_zero_remains_unbounded tests.test_omnivoice_tts.StudioLocalTests.test_run_command_timeout_fails_cleanly -v`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__`
  ...
  - `find . -type d -name __pycache__ -print`
- Tests:
  - Studio local timeout focused suite: PASS, 3 tests.
  - Full bridge validator: PASS, 147 tests, 1 skipped real-backend unittest
    smoke by default.
  - Strict real-backend acceptance with prepared Python adapter exports: PASS.
  - Real-backend smoke script with prepared Python adapter exports: PASS,
    generated a valid temporary WAV.
...
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - Studio HTTP health probes should always be bounded; Docker/Git command
    timeouts can remain opt-out for explicit operator-supervised runs.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
  - Continue bounded MVP hardening or install the bridge into the real Hermes
...
- Completed:
  - Rechecked repo state; branch was clean at commit `5ca2723`.
  - Rechecked default runtime state: no backend command, no Studio URL, no
    auto CLI by default, and one local designed profile present.
  - Closed the remaining command-backend output gap by passing command
    backends a private same-directory temporary output path instead of the
    requested final output path.
  - Kept the final output replacement atomic after validation and preserved
    `0600` permissions for successful generated audio.
  - Added regression coverage proving command backends receive the temp path,
    the temp file is removed after success, invalid backend output does not
    replace the final file, and no temp output is left behind.
  - Updated README, setup docs, custom voice docs, MVP handoff, and weekend
    summary to document validated command/Studio output replacement.
- Commands run:
  - `git status --short --branch`
  - `git log --oneline --decorate -6`
  - `python3 scripts/check-omnivoice-runtime.py --json`
  - `sed -n '300,560p' scripts/hermes-omnivoice-tts.py`
  - `python3 -m unittest tests.test_omnivoice_tts.OmniVoiceRegistryTests -v`
  - `python3 -m py_compile scripts/hermes-omnivoice-tts.py tests/test_omnivoice_tts.py`
  - `scripts/validate-omnivoice-bridge.sh`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && python3 scripts/omnivoice-acceptance.py --require-real-backend --json`
  - `eval "$(python3 scripts/setup-omnivoice-python-env.py --check-only --shell)" && scripts/test-omnivoice-tts.sh`
  - `rg -n "08:30 America/New_York|08:00 America/New_York|113 tests|111 tests" docs/omnivoice-mvp-handoff.md docs/omnivoice-weekend-summary.md README.md docs/omnivoice-acceptance.md`
  - `git diff --check`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__ && python3 scripts/check-omnivoice-artifacts.py --json`
  - `rm -rf tests/__pycache__ tests/fixtures/__pycache__ scripts/__pycache__ && find . -type d -name __pycache__ -print`
- Tests:
  - Targeted wrapper registry tests: PASS, 20 tests.
  - Python py_compile for the wrapper and tests: PASS.
  - `scripts/validate-omnivoice-bridge.sh`: PASS; includes 115 tests with 1
    expected opt-in real-backend skip, py_compile, strict package-file
    acceptance, fake-backend smoke, unconfigured smoke skip, secret-pattern
    scan, helper-backed generated-artifact scan, and `git diff --check`.
  - Strict real-backend acceptance after evaluating generated shell exports:
  ...
  - Real-backend smoke script after evaluating generated shell exports: PASS;
    generated a valid temporary WAV from the required smoke text.
  - Stale handoff snapshot scan: PASS; no 08:30/113-test or older summary state
    remains in the current handoff docs.
  - Repo artifact scan: PASS; no generated audio, models, local voice samples,
    env files, caches, local selection state, or pycache directories found.
- Blockers:
  ...
  - Studio live service remains blocked by the missing arm64 published image and
    source-build timeout noted in earlier heartbeats.
- Assumptions:
  - Custom command backends should not need the final output path; `{out}` can
    safely point at a wrapper-owned temporary file because Hermes receives the
    final requested output after validation.
  - Native-provider and in-app `/voice` command wiring still wait on the actual
    Hermes Agent source.
- Next action:
  - Commit the command-output temp-path hardening and keep the branch clean for
    handoff, or install the bridge into the real Hermes Agent checkout once the
    source path is available.
...
- Require local selection metadata to declare `provider: omnivoice` before it
  can be treated as selected OmniVoice voice state.
- Report selected voice current-state fields from the revalidated profile
