...
- Runtime profile loading rejects symlinked voice registry roots, voice
  directories, `voice.yaml` files, and cloned `ref_audio` files.
- Runtime diagnostics reuse the wrapper voice-profile validator when reporting
  local voice readiness.
- Create/import helpers refuse symlinked voice registry roots and final
  voice-directory symlinks so forced writes cannot alias another profile.
