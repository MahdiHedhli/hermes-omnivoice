...
- `scripts/create-omnivoice-voice.py clone` now rejects symlinked
  `--ref-audio` source inputs before copying a clone reference sample into the
  local registry.
- `scripts/import-omnivoice-studio-voice.py` now checks target voice directory
  ...
  design payloads no longer leave empty target directories behind.
...
- `scripts/hermes-omnivoice-python-adapter.py` now rejects non-finite or
  non-positive `--speed` values and non-positive `--sample-rate` values before
  loading the OmniVoice Python backend.
- `scripts/omnivoice-studio-local.py` now rejects invalid published `--port`
  values before Docker can run.
...
- Runtime diagnostics now validate the same backend command-template
  ...
  `real_backend_ready`.
- Runtime diagnostics now reject non-positive `--timeout` values before any
  Studio probe can reach lower-level URL handling.
- Acceptance now reports invalid runtime `--timeout` values as concise
...
- `scripts/find-hermes-source.py` and acceptance source discovery now reject
  non-positive scan timeouts, candidate/file limits, and negative max-depth
  values before walking the filesystem, keeping automated source readiness
  checks bounded.
...
- `scripts/hermes-omnivoice-voices.py preview` now rejects non-positive
  `--timeout` and non-positive or non-finite `--speed` overrides before
...
- `scripts/omnivoice-studio-local.py logs` now rejects negative `--tail`
  values before Docker can run.
...
- `scripts/create-omnivoice-voice.py` now rejects non-finite or non-positive
  ...
  reference audio.
- `scripts/create-omnivoice-voice.py` now rejects empty `--allowed-use` values
  before creating a profile directory or copying clone reference audio.
...
- `scripts/hermes-omnivoice-voices.py current` revalidates the selected profile
  ...
  stale selection metadata, and symlinked selection files are refused before
  reading.
...
- `scripts/create-omnivoice-voice.py clone` refuses symlinked `--ref-audio`
  source paths before validating or copying clone reference audio, so a helper
  invocation cannot silently import a local filesystem alias.
